...
public:
	...
	void iterativeRefinement(std::map<int, double>& eval_graph);
...
private:
	...
	const int LMethod(const std::map<int, double>& eval_graph, const int& cutoff);

	/* remove extremely dissimilar cluster merges */
	void removeExtreme(std::map<int, double>& eval_graph);
