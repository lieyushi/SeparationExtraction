...
Evrot::Evrot(const Eigen::MatrixXd& X, int method):
	mMethod(method),
	mNumDims(X.cols()),
	mNumData(X.rows()),
...
#pragma omp parallel for schedule(static) num_threads(8)
	...
	{
		int col=0;
		double mValue = mXrot.row(i).cwiseAbs().maxCoeff(&col);
		/*
		int row, col;
...
double Evrot::evqual(const Eigen::MatrixXd& X)
{
	// take the square of all entries and find max of each row
	Eigen::MatrixXd X2(X.rows(), X.cols());
#pragma omp parallel for schedule(static) num_threads(8)
	...
	Eigen::VectorXd max_values(X.rows());
...
double Evrot::evqualitygrad(const Eigen::VectorXd& theta, const int& angle_index)
{
	// build V,U,A
...
Eigen::MatrixXd Evrot::rotate_givens(const Eigen::VectorXd& theta)
{
	Eigen::MatrixXd G = build_Uab(theta, 0, mNumAngles-1);
	Eigen::MatrixXd Y = mX*G;
	return Y;
}

Eigen::MatrixXd Evrot::build_Uab(const Eigen::VectorXd& theta, const int& a, const int& b)
{
	int k,i;
	//set Uab to be an identity matrix
	Eigen::MatrixXd Uab(mNumDims,mNumDims);
	Uab.setZero();
	Uab.setIdentity();
...
Eigen::MatrixXd Evrot::gradU(const Eigen::VectorXd& theta, const int& k)
{
	Eigen::MatrixXd V(mNumDims,mNumDims);
	V.setZero();
